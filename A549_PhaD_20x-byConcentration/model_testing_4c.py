...
def get_key_from_value(d, value, ind):
    keys = []
    for k, v in d.items():
        if v == value:
            keys.append(k)  
...
model.load_weights(os.path.join(weights_path,"{}_clusters/DAC_weights/DAC_weights_{}clusters_iter1.h5".format(nb_classes,nb_classes)))
...
label_to_cluster = {}
true_labels = [0, 10, 50, 10, 100, 100]  # Replace with Transwell results
...
for i in range(nb_classes):
    labs = get_key_from_value(label_to_cluster, i, 0)
...
for category, value_list in zip(class_labels, predictions_Fe):
    data['Category'].extend([category] * len(value_list))
    data['Loss of Function (%)'].extend(value_list)
df = pd.DataFrame(data)
