...
def get_key_from_value(d, value, default):
...
model.load_weights(os.path.join(weights_path,"{}_clusters/DAC_weights/DAC_weights_{}clusters_iter2.h5".format(nb_classes,nb_classes)))
...
label_to_cluster = {}
true_labels = [0, 1, 25, 50, 100, 100]  # Replace with Transwell results
...
# Dictionary to store the highest frequency count for each cluster
...
for i in Ti_locs:  # labels corresponding to Ti
    ...
    most_frequent_cluster = sorted_clusters[0]
    
    if most_frequent_cluster not in label_to_cluster.values():
        label_to_cluster[i] = most_frequent_cluster
        cluster_to_count[most_frequent_cluster] = cluster_counts[most_frequent_cluster]
    else:
        existing_label = get_key_from_value(label_to_cluster, most_frequent_cluster, None)
        existing_count = cluster_to_count[most_frequent_cluster]
        if cluster_counts[most_frequent_cluster] > existing_count:
            label_to_cluster[i] = most_frequent_cluster
            cluster_to_count[most_frequent_cluster] = cluster_counts[most_frequent_cluster]
...
for i in Ti_locs:
    if label_to_cluster.get(i) is None:
        indices = np.where(y_test == i)[0]
        images = X_test[indices]
        preds = model.predict(images, verbose=1, batch_size=16)
        pred_cluster = np.argmax(preds, axis=1)
...
pred_label = np.argmax(cluster_counts)
label_to_cluster[i] = pred_label

sorted_damage_labels = []
for i in range(nb_classes):
    labs = get_key_from_value(label_to_cluster, i, None)
