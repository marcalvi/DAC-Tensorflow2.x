...
def get_data_batch(batch_size, mnist_data, mnist_labels):
    global nb_classes
    # Shuffle the samples and store the order
    indices = np.arange(mnist_data.shape[0])
    np.random.shuffle(indices)
    batch_data = mnist_data[indices]
    batch_labels = mnist_labels[indices]
    
    samples_per_class = int(batch_size // nb_classes)
    balanced_batch_data = []
    balanced_batch_labels = []

    for i in range(len(np.unique(y_train))):
        # Find indices of samples belonging to class c and store
        class_indices = np.where(batch_labels == i)[0]
        np.random.shuffle(class_indices)
        selected_indices = class_indices[:samples_per_class]
        balanced_batch_data.extend(batch_data[selected_indices])
        balanced_batch_labels.extend(batch_labels[selected_indices])

    balanced_batch_data = np.array(balanced_batch_data)
    balanced_batch_labels = np.array(balanced_batch_labels)

    # Shuffle the balanced batch data and labels together
    shuffle_indices = np.arange(len(balanced_batch_data))
    np.random.shuffle(shuffle_indices)
    balanced_batch_data = balanced_batch_data[shuffle_indices]
    balanced_batch_labels = balanced_batch_labels[shuffle_indices]

    return balanced_batch_data, balanced_batch_labels
...
# images_Ti_5 = []
# images_Ti_1 = []
# conditions_list = ["Control","1Fe","5Fe","10Fe","50Fe","100Fe","1Ti","5Ti","10Ti","50Ti","100Ti","1Sn","5Sn","10Sn","50Sn","100Sn"]
...
conditions_list = ["Control","100Fe","100Ti","100Sn"]
...
images_Ti_100 = []
images_control = []
images_list = [images_control, images_Fe_100, images_Ti_100, images_Sn_100]
...
# X_train_C = np.vstack((images_control, images_Sn_1, images_Sn_5, images_Sn_10, images_Sn_50, images_Sn_100))
# yC = np.zeros(len(images_control))
# y1 = 1 * np.ones(len(images_Sn_1))
# y5 = 2 * np.ones(len(images_Sn_5))
# y10 = 3 * np.ones(len(images_Sn_10))
# y50 = 4 * np.ones(len(images_Sn_50))
# y100 = 5 * np.ones(len(images_Sn_100))
# y_train_C = np.hstack((yC, y1, y5, y10, y50, y100)).astype(int)

# Creating train dataset for materials and concentration
X_train_M = np.vstack((images_control, images_Fe_100, images_Ti_100, images_Sn_100))

# Creating train labels for materials and concentration
yC = np.zeros(len(images_control))
yFe = 1 * np.ones(len(images_Fe_100))
yTi = 2 * np.ones(len(images_Sn_100))
ySn = 3 * np.ones(len(images_Ti_100))

y_train_M = np.hstack((yC, yFe, yTi, ySn)).astype(int)
...
wandb.init(project="DAC_immunos_M",
           # track hyperparameters and run metadata with wandb.config
           config={"nb_clusters": nb_classes,"BN momentum": 0.9,"BN learnable": trainBN,"batch_size": batch_size,
...
# paths
path = os.path.join(r"/Users/marcalbesa/Desktop/TFG/xarxa/DAC-MarcAlbesa/immunos_TFG/DAC_immnos_material", "%d_clusters" %nb_classes)
...
# model.load_weights(os.path.join(path_weights,"DAC_weights_{}clusters_epoch20.h5".format(nb_classes)))

# Perform training loop under condition
for e in range(total_epochs):
    np.random.shuffle(index)
    
    for i in range(X_train.shape[0]//nb): #numero de 62 que hiha al dataset
        data_samples,data_labels = get_data_batch(100, X_train, y_train)
    ...
    iteration = 0
    # data_samples, data_labels = get_data_batch(130, X_train, y_train)
    pred_cluster = []
    pred_labels = []
    batches = 4
    len_batch = int(len(X_train)/batches)
    for i in range(batches):
        Xbatch = X_train[i*len_batch:(i+1)*len_batch]
    ...
    if epoch%2 ==0:
        ...
        model.save_weights(os.path.join(path_weights,"DAC_weights_{}clusters_epoch{}.h5".format(nb_classes,epoch)))
        ...
        cluster_l1,cluster_l2,model = ConvNetwork(inp_shape, nb_classes)
        model.compile(optimizer=RMSprop(lr), loss=["binary_crossentropy", loss_function], loss_weights=[1,10])
        model.load_weights(os.path.join(path_weights,"DAC_weights_{}clusters_epoch{}.h5".format(nb_classes,epoch)))
...
gif_path = os.path.join(path,"learning_gif_iter{}.gif".format(code_iteration)) 
frame_rate = 2.5
