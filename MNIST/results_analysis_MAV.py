...
nb_classes = 10
path = "/Users/marcalbesa/Desktop/TFG/xarxa/DAC-MarcAlbesa/mnist/"
        ...
        with h5py.File(path_out_file, 'r') as hf:
            ...
            if first:
                for i in range(nb_classes):
                    ...
                    th = 0.7 if nb_classes>6 else 0.5
...
def plot_histograms(predicted_clusters_labels, nb_classes):
    ...
    for idx, (cluster_index, cluster_labels) in enumerate(predicted_clusters_labels.items()):
        ...
        if nb_classes >=6:
            bar_positions = range(10)
            label_counts = [np.sum(cluster_labels == i) for i in range(10)]
            axs[row, col].bar(bar_positions, label_counts, color='blue', alpha=0.7, width=0.7, align='center')
            highest_freq_column = np.argmax(label_counts)
            axs[row, col].bar(bar_positions[highest_freq_column], label_counts[highest_freq_column], color='red', alpha=0.7, width=0.8, align='center')
            axs[row, col].set_title(f"Cluster {cluster_index}")
            axs[row, col].set_xlabel('Label')
            axs[row, col].set_xticks(range(10))
            axs[row, col].set_yticks([])
            axs[row, col].set_yticklabels([])
            axs[row, col].grid(False, axis='x')  # Disable vertical grid lines

        if nb_classes == 5:
            bar_positions = range(10)
            label_counts = [np.sum(cluster_labels == i) for i in range(10)]
            axs[col].bar(bar_positions, label_counts, color='blue', alpha=0.7, width=0.7, align='center')
            highest_freq_column = np.argmax(label_counts)
            axs[col].bar(bar_positions[highest_freq_column], label_counts[highest_freq_column], color='red', alpha=0.7, width=0.8, align='center')
            axs[col].set_title(f"Cluster {cluster_index}")
            axs[col].set_xlabel('Label')
            axs[col].set_xticks(range(10))
            axs[col].set_yticks([])
            axs[col].set_yticklabels([])
            axs[col].grid(False, axis='x')  # Disable vertical grid lines

    
    fig.suptitle("Histograms of clustering performance across iterations", fontsize=16)  # Add title for the entire plot
    output_path = os.path.join(path, "MNIST_{}clusters.png".format(nb_classes))
...
def find_optimum_clusters(n_min,n_max):
    global predicted_clusters_labels, std_dev,cluster_labels,label_counts,filtered_counts_list
    sds_inter = {}
    for nb_classes in range(n_min,n_max+1):
        ...
        for cluster_labels in predicted_clusters_labels.values():
            label_counts = np.array([np.sum(cluster_labels == i) for i in range(10)])
            norm_factor = np.max(label_counts)
            filtered_norm_counts = (label_counts[label_counts <= 0.9* np.max(label_counts)])/norm_factor
...
opt_clusters = find_optimum_clusters(5,10)
